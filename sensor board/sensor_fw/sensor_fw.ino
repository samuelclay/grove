...
int PIRcount = 0; //Used to keep track of how long the PIR sensor is active
...
int prox_window[10] = {0}; //Averaging window is 10 samples wide
int indx = 0;
...
int closePos = 35; // recommended servo angle to close the flower. (previous values used: 40)
int openPos = 110; // recommended servo angle to open the flower (previous values used: 130)
...
void setup() {
  ...
  myservo.attach(3); //set up the servo on pin 3
  ...
  //Set up servo output
  pinMode(3, OUTPUT);
  myservo.write(closePos); //Close flower to start

  //Setup PIR input
 pinMode(20, INPUT_PULLUP);

  Serial.print("Calibrating breath sensor...");

  for(int i=0;i<=49;i++){
    Serial.print("Settling ");
    getwind();
  }
  WindThresh = getwind() + 25;
...
if (alarm == LOW) {
  if (PIRcount > 0){
    PIRcount--; //If we haven't seen anything, decrement PIRcount. This will act as a filter to handle noise. 
    //If we're seeing more actives than inactives, then the count will go up and we'll eventually trip to open the flower.
    //If we're seeing more inactives than actives, then the count will go down and we'll eventually zero out.
  }
  if (PIRcount == 0){
    //It's been at least 50 loops since we last saw movememnt, so close the flower
    myservo.write(closePos); // This closes the flower.
    Serial.println("Closing flower");
    digitalWrite(2, LOW); //tell main board we saw outer prox thresh
  }
  //Serial.println("PIR is LOW");
  //digitalWrite(16, LOW); //Write comm pin to main board to tell it we don't see a breath.
} else {
...
//Now let's check the other prox sensor
    ...
    while(getprox(100000000)){ //Threshold for deciding we've seen a person
        ...
        int winsum = 0;
        for(int j=0;j<20;j++){
          winsum = winsum + windvec[j];
        }
        int winavg = winsum/20;
      if(getwind() > winavg+15){ //We're seeing a breath
        digitalWrite(0, LOW); //tell main board we saw a breath
        Serial.println("Saw breath");
      } else {
        digitalWrite(0, HIGH); //tell main board we haven't seen anything
      }
    }
      digitalWrite(1, LOW);
...
//  Serial.print("   WindSpeed MPH ");
  ...
  strip.setPixelColor(1, min(windColor, 255));
...
int analyze(float wind){
  //This is a simple threshold for now. 
  if (wind > WindThresh){
    return 1;
  } else {
    return 0;
